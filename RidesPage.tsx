...
const RidesPage: React.FC = () => {
  ...
  // ✅ تحميل الرحلات من Firebase في الوقت الحقيقي
  useEffect(() => {
    if (!user?.id) { setLoading(false); return; }

    const unsub = listenToUserRides(user.id, (rides) => {
      // دمج مع الرحلات المحلية وإزالة التكرار
      rides.forEach((ride) => {
                ...
                {ride.driver && (
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-dark-100 dark:border-dark-700">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-dark-700 dark:text-dark-200 truncate">{ride.driver.name}</p>
                      <p className="text-xs text-dark-400">{ride.driver.vehicleModel}</p>
                    </div>
                    <div className="flex items-center gap-0.5">
                      <Star size={10} className="text-primary-500 fill-primary-500" />
                      <span className="text-xs font-bold text-dark-600 dark:text-dark-300">{ride.driver.rating}</span>
                    </div>
                  </div>
                )}
