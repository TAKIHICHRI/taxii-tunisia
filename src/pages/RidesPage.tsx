...
import { Star, MapPin, Clock, Car, FileText, Share2 } from 'lucide-react';
...
const RidesPage: React.FC = () => {
  ...
  const { rideHistory, addToast } = useAppStore();
  ...
  return (
    <div className="min-h-dvh bg-dark-50 dark:bg-dark-900 safe-top pb-24">
      ...
      <div className="bg-white dark:bg-dark-800 px-5 pt-5 pb-4 border-b border-dark-100 dark:border-dark-600">
        <h1 className="font-extrabold text-dark-900 text-xl mb-4">{t('rideHistory')}</h1>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-bold text-dark-800 text-sm">{t(ride.rideType)}</p>
                      <p className="text-dark-400 text-xs">{new Date(ride.createdAt).toLocaleDateString('ar-TN')}</p>
                    </div>
                  </div>
                  <div className="text-end">
                    <p className="font-extrabold text-dark-900">{ride.price} {t('currency')}</p>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${status.bg} ${status.text}`}>
                      {t(status.label)}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-dark-600 truncate">{ride.pickup.name || ride.pickup.address}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-red-500" />
                    <span className="text-sm text-dark-600 truncate">{ride.destination.name || ride.destination.address}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-dark-100">
                  <div className="flex items-center gap-3 text-xs text-dark-400">
                    ...
                    <span className="flex items-center gap-1"><MapPin size={12} /> {ride.distance} {t('km')}</span>
                  </div>
                  {ride.rating && (
                ...
                {ride.driver && (
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-dark-100">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-dark-700 truncate">{ride.driver.name}</p>
                      <p className="text-xs text-dark-400">{ride.driver.vehicleModel}</p>
                    </div>
                    <div className="flex items-center gap-0.5">
                      <Star size={10} className="text-primary-500 fill-primary-500" />
                      <span className="text-xs font-bold text-dark-600">{ride.driver.rating}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-2 mt-3 pt-3 border-t border-dark-100">
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-primary-50 dark:bg-primary-900/20 text-primary-600 text-xs font-medium"
                  >
                    <Share2 size={14} />
                    {t('shareRide')}
                  </button>
      ...
      {invoiceRide && (
        <RideInvoice ride={invoiceRide} onClose={() => setInvoiceRide(null)} />
      )}
    </div>
  );
};
