    ...
    return (
      <div className="min-h-dvh bg-white dark:bg-dark-900 flex flex-col items-center justify-center px-6 safe-top">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="font-extrabold text-dark-900 dark:text-white text-2xl mb-3 text-center">
          تم استلام طلبك! 🎉
        </motion.h2>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
          className="text-dark-500 dark:text-dark-400 text-center mb-8">
          سنتواصل معك خلال 24-48 ساعة بعد مراجعة طلبك
        </motion.p>
      ...
      <div className="min-h-dvh bg-white dark:bg-dark-900 safe-top safe-bottom">
        <div className="relative bg-gradient-to-br from-primary-500 to-primary-600 px-6 pt-12 pb-10">
          ...
          <div className="text-center pt-6">
            ...
            <p className="text-white/70 text-sm mt-2">انضم لأسرة سائقي Alou في القصرين</p>
          </div>
        </div>
        <div className="px-5 py-6 space-y-4">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <div>
                <p className="font-bold text-dark-800 dark:text-white text-sm">{item.title}</p>
                <p className="text-dark-500 dark:text-dark-400 text-xs mt-0.5">{item.desc}</p>
              </div>
            </motion.div>
          ))}
  ...
  return (
    <div className="min-h-dvh bg-white dark:bg-dark-900 safe-top safe-bottom">
      ...
      <form onSubmit={handleSubmit} className="px-5 pb-8 pt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="border-2 border-dashed border-dark-200 dark:border-dark-600 rounded-2xl p-4 flex flex-col items-center gap-2">
            <Upload size={20} className="text-dark-400" />
            <span className="text-xs text-dark-500 dark:text-dark-400 font-medium text-center">صورة رخصة القيادة</span>
            <span className="text-[10px] text-dark-400">قريباً</span>
          </div>
          <div className="border-2 border-dashed border-dark-200 dark:border-dark-600 rounded-2xl p-4 flex flex-col items-center gap-2">
            <Upload size={20} className="text-dark-400" />
            <span className="text-xs text-dark-500 dark:text-dark-400 font-medium text-center">صورة السيارة</span>
            <span className="text-[10px] text-dark-400">قريباً</span>
          </div>
        </div>
